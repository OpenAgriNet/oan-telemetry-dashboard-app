...
const scorePresentation = (value: number | null) => {
  ...
  if (value >= 4.5) return { label: "Excellent", badge: "border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400", fill: "bg-emerald-500", glow: "border-emerald-500/20 hover:border-emerald-500/40" };
  if (value >= 3.5) return { label: "Strong", badge: "border-blue-500/30 bg-blue-500/10 text-blue-600 dark:text-blue-400", fill: "bg-blue-500", glow: "border-blue-500/20 hover:border-blue-500/40" };
  if (value >= 2.5) return { label: "Acceptable", badge: "border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400", fill: "bg-amber-500", glow: "border-amber-500/20 hover:border-amber-500/40" };
  if (value >= 1.5) return { label: "Needs work", badge: "border-orange-500/30 bg-orange-500/10 text-orange-600 dark:text-orange-400", fill: "bg-orange-500", glow: "border-orange-500/20 hover:border-orange-500/40" };
  return { label: "Critical", badge: "border-destructive/30 bg-destructive/10 text-destructive", fill: "bg-red-500", glow: "border-destructive/30 hover:border-destructive/50" };
};
const ScoreScale = ({ value, color }: { value: number | null; color: string }) => {
  const filled = value == null ? 0 : Math.max(0, Math.min(5, Math.round(value)));
  return <div className="mt-2 flex gap-1.5" aria-label={value == null ? "Not scored" : `${value.toFixed(2)} out of 5`}>
    {[1, 2, 3, 4, 5].map((level) => <span key={level} className={`h-2 flex-1 rounded-sm ${level <= filled ? color : "bg-muted"}`} />)}
  </div>;
};
